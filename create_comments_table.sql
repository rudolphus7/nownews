...
-- RPC for incrementing comment reactions
CREATE OR REPLACE FUNCTION increment_comment_reaction(comment_id UUID, reaction_type TEXT)
RETURNS void AS $$
BEGIN
    IF reaction_type = 'likes' THEN
        UPDATE public.comments SET likes = likes + 1 WHERE id = comment_id;
    ELSIF reaction_type = 'dislikes' THEN
        UPDATE public.comments SET dislikes = dislikes + 1 WHERE id = comment_id;
    END IF;
END;
