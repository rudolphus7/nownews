...
module.exports = async (req, res) => {
    ...
    if (action === 'generate-fb') {
        const prompt = `Ти — професійний редактор українського новинного видання "BUKVA NEWS". Твоє завдання — написати короткий опис до статті для публікації у Facebook.

Вимоги до поста:
1. Формат: Напиши опис як анотацію до цікавого фільму (2-4 лаконічні, але змістовні речення).
2. Тон: Без дешевого клікбейту (без слів "шок", "тривожна реальність", "ви не повірите"). Пиши серйозно, зі збереженням журналістської етики, але так, щоб суть проблеми чіпляла читача.
3. Емоції: Додай 1-2 стримані емодзі (наприклад: 📌, 📍, 🏗️, 📋), тільки якщо вони доречні.
4. Call-to-action (заклик до дії): В кінці коротко та органічно закликай прочитати повну новину за посиланням.
5. Посилання: Обов'язково встав посилання на статтю в самому кінці.
